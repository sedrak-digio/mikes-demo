...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    try {
        ...
        const countersRef = await dbClient.item('hungry-counter');
        ...
        if(counters) {
            await countersRef.replace({ id: 'hungry-counter', counter: counterValue })
        } else {
            await dbClient.items.create({id: 'hungry-counter', counter: counterValue})
        }
